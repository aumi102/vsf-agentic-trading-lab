...
| PIT breadth v2 | Draft: 10 target issuers, 8 sectors, 23 total rows, 10 credible comparable statement rows, 6 credible unique events, 3 comparable issuers/sectors, 13 non-credible rows including 3 blocked and 2 network_error; `pit_inconclusive` |
...
- PIT breadth v2 has comparable rows for FPT, VCI, and HPG. KDH Q1, ACB
  ...
  `publicDate`. ACB/DGC are metadata-only `legacy_browser_ua_diagnostic`
  blocker rows because those commands used the script default browser-style
  User-Agent.
