...
The current DeepAgents service exposes these read-only tools over QuestDB and deterministic research code:
...
- `get_ohlcv_window`
- `run_ma_backtest` — only wired for execution when the user explicitly asks for backtest/simulate/strategy performance.
...
4. `BacktestResearchAgent`: controlled research backtests only when explicitly requested.
5. `RiskGuardrailAgent`: domain routing, unavailable-domain handling, no-advice and no-hallucination checks.
6. `AnswerComposerAgent`: concise final answer with dates, source tables, caveats, and tool evidence.
...
flowchart TD
    ...
    R -->|explicit backtest only| BT[BacktestResearchAgent]
...
| Backtest/research simulation | `run_ma_backtest`, required market-data tools | FA/event tools unless explicitly part of the research question | Only run when user says backtest/simulate/strategy performance/run strategy. |
