...
| Controlled factor source verification | No-network plan-only and local-payload verification layer reuses the source adapter to confirm a payload yields usable, provenance-backed factor records on a small explicit symbol set. No approved live source, no full-universe crawl, no adjusted OHLC population, no DB mutation. |
...
- **Adjusted OHLC hardening:** Schema slots and pure helpers exist, but trusted corporate-action factor sourcing is not implemented. Backtrader VN100 runs remain blocked until adjusted OHLC rows are populated and validated.
...
12. Verify an approved adjusted-close or corporate-action source on a small explicit symbol set, then use the local factor application path before Backtrader VN100 runs.
