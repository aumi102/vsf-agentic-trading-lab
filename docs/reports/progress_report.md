...
- Main blocker: safe fetch policy, price adjustment semantics, corporate actions, FA metric mapping coverage remains below 95% gate and conflicts require integration strategy, `publicDate` PIT validation, full-history FA fetch policy, and tool contracts.
...
- [ ] Metric mapping coverage below 95% gate — best observed union (VCI+VCB+BVH+SSI): BS 89.4% / IS 92.3% / CF 86.7%; 88 conflicts require integration strategy; `line_item_name` integration into parser not implemented; `line_item_name` empty for all parsed rows.
