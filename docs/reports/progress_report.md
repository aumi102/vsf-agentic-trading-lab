...
- Main blocker: FA metric mapping coverage below 95% gate; 88 conflicts; Option C resolver and firm-type determination logic implemented but parser integration is not implemented; `publicDate` PIT validation; full-history FA fetch not implemented; DB write blocked; backtest blocked.
...
- [x] FA Option C mapping resolver implemented: pure offline `scripts/resolve_vietcap_iq_fa_metric_mapping.py` (74 tests, 487 total passing); 6 statuses (`primary`, `consensus_fallback`, `conflict_skipped`, `not_covered`, `no_mapping_available`, `section_mismatch`); parser not yet integrated; see `docs/data_sources/vietcap_iq_fa_mapping_resolver_tests.md`.
- [ ] FA mapping parser integration not implemented — resolver and firm-type logic designed; parser code change required before `line_item_name_en`/`line_item_name_vi` can be populated.
- [ ] Metric mapping coverage below 95% gate — best observed union (VCI+VCB+BVH+SSI): BS 89.4% / IS 92.3% / CF 86.7%; 88 conflicts; `line_item_name` empty for all parsed rows.
