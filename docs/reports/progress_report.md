...
| OHLCV ingestion foundation | Cached ingestion foundation records `source_runs`, per-run `raw_source_payloads`, and `ingestion_watermarks`; refreshes canonical prices/features/signals by symbol; live-gated attempts are auditable but live fetch remains not implemented pending mentor source/DB decision. |
...
| OHLCV ingestion foundation tests | 20 targeted tests |
