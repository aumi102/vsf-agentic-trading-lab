...
| Mentor local demo UI | Local-only visual dashboard launched with one command. Uses synthetic fixtures with no network, DB mutation, or real strategy execution. |
| Mentor decision capture | Browser-only form previews a pending adjusted-OHLC strategy contract draft. No persistence, execution, family enablement, or advice. |
| Mentor decision capture polish | Client-side completeness warnings, sample fill, and a copyable boundary talk track. Blank numbers remain null/not-ready; drafts remain pending. |
| Mentor demo readiness polish | Read-only readiness checks and a five-minute call script verify the command, minimal uploads, noop-only registry, pending decisions, and safety boundaries. |
