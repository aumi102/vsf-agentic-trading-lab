...
- Main blocker: FA metric mapping coverage below 95% gate; 88 conflicts; integration strategy is designed (Option C) but parser integration is not implemented; `publicDate` PIT validation; full-history FA fetch not implemented; DB write blocked; backtest blocked.
...
- [ ] FA mapping integration not implemented — strategy is designed; parser code change and tests required before `line_item_name_en` can be populated.
