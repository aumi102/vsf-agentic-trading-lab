...
- Main blocker: FA metric mapping coverage below 95% gate; 88 conflicts; integration strategy and firm-type determination logic designed (Option C + Approach D) but parser integration is not implemented; `publicDate` PIT validation; full-history FA fetch not implemented; DB write blocked; backtest blocked.
...
- [ ] FA mapping integration not implemented — strategy and firm-type logic designed; parser code change and tests (§6.6) required before `line_item_name_en` can be populated.
