...
| Official Disclosure Foundation v1 | Live-verified: FPT IR 20 bronze records (quality=pass, pit=date_only_available); HOSE=js_app_shell; HNX=timeout; VCI IR domain unresolved (`vietcapital.com.vn` is VCAM) |
| Test suite | **652 tests pass** (71 integration + 74 resolver + 54 firm-type + 19 PIT validator + 67 disclosure + others) |
| DB write | **Blocked** |
| Backtest | **Blocked** |
...
| Disclosure foundation tests | 67 (+ FPT HTML parser, js_app_shell classify, activation/config, FPT execute integration) |
| Total tests passing | 652 |
...
4. ~~Build official disclosure source discovery/crawler POC for HOSE/HNX/company IR~~ — Foundation v1 live-verified; FPT IR parsed (20 records); HOSE=js_app_shell; HNX=timeout; VCI IR domain unresolved. Resolve VCI domain to advance PIT gate.
