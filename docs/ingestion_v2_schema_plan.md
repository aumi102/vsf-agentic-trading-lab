...
- **Vietcap IQ financial statements / FA data** — current status summary:
  ...
  - **Blocked:** full-history FA fetch not implemented; DB write blocked (union mapping coverage < 95% + PIT unconfirmed + schema not designed + integration not implemented); backtest blocked; `publicDate` PIT semantics unconfirmed. Total: 413 tests pass.
