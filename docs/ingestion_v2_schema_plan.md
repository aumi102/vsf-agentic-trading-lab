...
- **Vietcap IQ financial statements / FA data** — access unblocked: FA direct clean-profile diagnostic (run `20260609T035318Z`) returned `200 JSON` for `VCI/BALANCE_SHEET` with a fresh 8-header `httpx.Client` (no warm-up, no Cookie, no sec-ch-ua*); raw payload captured (`byte_size=354443`, `data_keys=quarters,years`); payload-shape review written at `docs/data_sources/vietcap_iq_fa_payload_shape_review.md`; format is wide-format with 331 opaque metric codes, 33 quarterly + 8 annual rows, `publicDate` field present for all rows; no parser, DB write, or backtest implemented; PIT availability of `publicDate` field unconfirmed; next steps are to probe `INCOME_STATEMENT` and one additional symbol (`FPT`) before implementing a dry-run parser.
