...
- **Vietcap IQ financial statements / FA data** are a separate source-discovery gate after the controlled OHLCV proof; FPT endpoint candidates exist, but non-secret source-probe attempts returned `403/auth_required`; VCI `httpx` browser-session warm-up diagnostic (run `20260609T024007Z`) confirmed `trading.*` public endpoints return `200` while all `iq.*` endpoints return `403/auth_required`; search-bar parity diagnostic (run `20260609T032924Z`) shows a fresh `httpx.Client` with 8-header profile returns `200 JSON` from `iq.vietcap.com.vn`, search-bar parity (run `20260609T032924Z`) and FA direct clean-profile (run `20260609T035318Z`) diagnostics both returned `200 JSON` with a fresh `httpx.Client` and 8-header profile (no warm-up, no `Cookie`, no `Authorization`, no `sec-ch-ua*`), suggesting warm-up session state was the contributing factor for earlier `403` results; the FA endpoint (`VCI/financial-statement?section=BALANCE_SHEET`) returned `data_keys=quarters,years`, `byte_size=354443`; raw payload captured for shape inspection; parser planning can advance to payload-shape review but no parser, DB write, or backtest is implemented yet.
