...
- Full-history feasibility is still an open gate: FPT, VNM, and VCB `countBack=5000` probes show gap-chart can return large historical windows with stable shape, but older `accumulatedValue` completeness degrades before `2022-09-15`; the next step is a saved-payload parser dry run, not sector-batched fetching.
