...
- Full-history feasibility is still an open gate: the verified `countBack=250` gap-chart payload is recent history only, while the FPT-only `countBack=500` probe is first evidence that gap-chart can return more than the 250-bar window; larger FPT-only probes must still confirm whether full-history or large-window re-fetch is practical before sector-batched OHLCV fetching.
