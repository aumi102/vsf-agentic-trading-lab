...
- Full-history feasibility is still an open gate: FPT-only `countBack=500`, `1000`, `2000`, and `5000` probes show gap-chart can return larger windows, with `countBack=5000` appearing capped by available FPT history; older `accumulatedValue` completeness degrades before `2022-09-15`, so VNM/VCB large-window repeats must still confirm cross-symbol behavior before sector-batched OHLCV fetching.
