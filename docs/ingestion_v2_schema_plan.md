...
- **Vietcap IQ financial statements / FA data** are a separate source-discovery gate after the controlled OHLCV proof; FPT endpoint candidates exist, but the first non-secret source probe returned `403/auth_required`, so no row-level sample, parser, DB write, or backtest exists yet.
