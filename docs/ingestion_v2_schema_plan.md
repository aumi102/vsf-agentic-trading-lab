...
- **Vietcap IQ financial statements / FA data** — access unblocked and shape cross-checked: FA direct clean-profile diagnostic returned `200 JSON` for VCI BALANCE_SHEET (run `20260609T035318Z`), VCI INCOME_STATEMENT (run `20260609T075846Z`), and FPT BALANCE_SHEET (run `20260609T075857Z`); initial section/symbol cross-check passed (three tested cases, same clean 8-header profile); payload-shape review at `docs/data_sources/vietcap_iq_fa_payload_shape_review.md`, cross-check at `docs/data_sources/vietcap_iq_fa_shape_cross_check.md`; key finding: `nos*` columns are entirely null for non-securities firms (null ≠ 0.0 distinction required); parser dry-run implemented (`scripts/parse_vietcap_iq_fa_payloads_dry_run.py`): `34,563` long-format fact rows from 3 saved payloads, null/zero distinction preserved; `line_item_name` empty (no mapping); no DB write, no backtest; PIT availability of `publicDate` unconfirmed; see `docs/data_sources/vietcap_iq_fa_parser_dry_run.md`; metric mapping discovery ran — no local mapping found, live probe of `/financial-statement/metrics` inconclusive (DNS failure, run `20260609T091305Z`); see `docs/data_sources/vietcap_iq_fa_metric_mapping_discovery.md`; parser hardened: 7 validation checks added (duplicate keys, `publicDate` format, mapping coverage, nos* null pattern, `value_status` validity, no invented names, metric column detection); `--strict` mode and deterministic sort added; 30 new tests (235 total); see `docs/data_sources/vietcap_iq_fa_parser_hardening.md`; **FA ingestion V2 readiness package added**: readiness document at `docs/data_sources/vietcap_iq_fa_ingestion_v2_readiness.md` defines all confirmed facts, open gates, policies, and DB/backtest readiness gates; manifest planner `scripts/plan_vietcap_iq_fa_full_history_manifest.py` generates a deterministic fetch-plan CSV (dry-run only, no network); 43 new manifest tests added (278 total passing); **FA mapping and CASH_FLOW probe package added**: metric mapping re-probe succeeded (run `20260610T025420Z`, HTTP 200, 1078 codes across BS/IS/CF/NOTE sections); coverage 62.8% BS / 43.6% IS / 65.8% CF — below 95% gate threshold; CASH_FLOW confirmed (VCI + FPT, HTTP 200, 33Q + 8Y, 225 codes, `publicDate` non-null); mapping dry-run parser `scripts/parse_vietcap_iq_fa_metric_mapping_dry_run.py` added (offline, no network, no DB, 50 tests); probe report at `docs/data_sources/vietcap_iq_fa_mapping_cashflow_probe.md`; full-history FA fetch is **planned, not implemented**; DB write remains **blocked** (mapping coverage below threshold + PIT + schema gates); backtest remains **blocked**; metric mapping is **partially retrieved but below 95% coverage gate**; `publicDate` PIT remains **unconfirmed**.
