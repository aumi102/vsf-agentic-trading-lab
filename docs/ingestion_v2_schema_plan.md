...
- **Vietcap IQ financial statements / FA data** — current status summary:
  - *Access confirmed:* HTTP 200 JSON (clean 8-header profile, no Cookie/Auth) for VCI BS, VCI IS, FPT BS (2026-06-09); VCI CF, FPT CF (2026-06-10). `nos*` null for non-securities firms (null ≠ 0.0 must be preserved). See `docs/data_sources/vietcap_iq_fa_payload_shape_review.md`, `vietcap_iq_fa_shape_cross_check.md`.
  - *Parser dry-run:* `scripts/parse_vietcap_iq_fa_payloads_dry_run.py`; 34,563 long-format fact rows from 3 saved payloads; null/zero distinct; `line_item_name` empty; no DB write; no backtest. See `docs/data_sources/vietcap_iq_fa_parser_dry_run.md`.
  - *Parser hardened:* 7 validation checks (duplicate keys, `publicDate` format, mapping coverage, `nos*` pattern, `value_status` validity, no invented names, metric columns); `--strict` mode; deterministic sort; 235 tests total. See `docs/data_sources/vietcap_iq_fa_parser_hardening.md`.
  - *Readiness package:* readiness doc (`docs/data_sources/vietcap_iq_fa_ingestion_v2_readiness.md`) defines all confirmed facts, open gates, and policies; manifest planner `scripts/plan_vietcap_iq_fa_full_history_manifest.py` (dry-run, no network); 43 manifest tests (278 total at that point).
  - *Metric mapping re-probe (2026-06-10):* `/financial-statement/metrics` returned HTTP 200; 1078 non-null codes across BALANCE_SHEET / INCOME_STATEMENT / CASH_FLOW / NOTE; VCI-only coverage 62.8% BS / 43.6% IS / 65.8% CF — **below 95% DB write gate threshold**; mapping dry-run parser `scripts/parse_vietcap_iq_fa_metric_mapping_dry_run.py` added (offline, 50 tests); `line_item_name` integration not implemented. See `docs/data_sources/vietcap_iq_fa_mapping_cashflow_probe.md`.
  - *CASH_FLOW confirmed:* VCI and FPT HTTP 200; 33Q + 8Y; 225 codes per row; `publicDate` non-null. Same envelope as BS/IS.
  - *Union mapping — bank/insurance probe (2026-06-10):* VCB (bank), BVH (insurance), SSI (securities) mapping payloads retrieved; mapping is firm-type-specific (SSI = VCI; VCB and BVH return different codes); union of 4 firm types = 1793 codes, 88 name conflicts (4.9%); **union coverage: BS 89.4% / IS 92.3% / CF 86.7% — still below 95% gate**; union analysis script `scripts/analyze_vietcap_iq_fa_metric_mapping_union.py` added (offline, 39 tests, 367 total passing); `line_item_name` integration not implemented. See `docs/data_sources/vietcap_iq_fa_mapping_coverage_bank_probe.md`.
  - *Integration strategy (2026-06-10):* Option C (Hybrid gated mapping) documented in `docs/data_sources/vietcap_iq_fa_mapping_integration_strategy.md`; per-symbol primary + union consensus fallback; provenance columns designed (`line_item_name_en`, `mapping_status`, `mapping_source_symbol`, `mapping_source_run_id`, `mapping_conflict`).
  - *Firm-type determination (2026-06-10):* Approach D (Hybrid metadata-primary) documented in `docs/data_sources/vietcap_iq_fa_firm_type_determination.md`; uses `company_type_code` from Vietcap IQ universe CSV (`NH`=bank, `BH`=insurance, `CK`=securities, others=general) + explicit overrides for 4 probed symbols; planner script `scripts/plan_vietcap_iq_fa_firm_type_mapping.py` (46 tests).
  - *Option C resolver (2026-06-10):* Pure offline Option C mapping resolver `scripts/resolve_vietcap_iq_fa_metric_mapping.py` (74 tests); resolves `(section, line_item_code)` → `MappingResult` with 6 statuses (`primary`, `consensus_fallback`, `conflict_skipped`, `not_covered`, `no_mapping_available`, `section_mismatch`).
  - *Parser mapping integration (2026-06-10):* Resolver wired into `scripts/parse_vietcap_iq_fa_payloads_dry_run.py`; 7 new output columns (`line_item_name_en`, `line_item_name_vi`, `mapping_status`, `mapping_source_symbol`, `mapping_source_run_id`, `mapping_conflict`, `mapping_group`); 65 new integration tests; dry-run on 5 saved payloads (VCI BS/IS/CF + FPT BS/CF) = 53,013 fact rows, 0 errors; backward compatible (no-mapping-input path unchanged); `line_item_name` (legacy) still empty; see `docs/data_sources/vietcap_iq_fa_parser_mapping_integration.md`.
  - **Blocked:** full-history FA fetch not implemented; DB write blocked (union mapping coverage < 95% + PIT unconfirmed + schema not designed); backtest blocked; `publicDate` PIT semantics unconfirmed. Total: 552 tests pass.
