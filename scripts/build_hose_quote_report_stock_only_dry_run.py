...
def write_outputs(*, outputs: dict[str, Any], output_dir: Path) -> None:
    ...
    (ROOT / "docs/hose_quote_report_stock_only_filter_review.md").write_text(build_review_doc(outputs["summary"]), encoding="utf-8")
...
def build_review_doc(summary: dict[str, Any]) -> str:
    examples = ", ".join(f"`{symbol}`" for symbol in summary["excluded_symbol_examples"][:40])
    return f"""---
...
- Quote-report directory: `{summary['quote_report_dir']}`
- Listed-universe directory: `{summary['listed_universe_dir']}`
...
`{summary['quote_report_dir']}/stock_only/`
