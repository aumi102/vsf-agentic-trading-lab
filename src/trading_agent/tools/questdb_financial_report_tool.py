...
def get_latest_financial_report(symbol: str, statement_type: str | None = None, url: str = DEFAULT_URL) -> dict[str, Any]:
    ...
    for section, table in targets:
        ...
        sql = (
            ...
            f"AND public_date = (SELECT max(public_date) FROM {table} WHERE symbol = '{sym}') "
    ...
    out["tool_calls"] = [{"tool": "get_latest_financial_report", "args": {"symbol": sym, "statement_type": statement_type}, "status": "ok", "row_count": len(rows)}]
    return out
...
def get_financial_metrics(symbol: str, statement_type: str, metric_codes: list[str] | None = None, limit: int = 50, url: str = DEFAULT_URL) -> dict[str, Any]:
    ...
    sql = f"SELECT * FROM {table} WHERE symbol = '{sym}'{code_filter} ORDER BY public_date DESC, metric_code LIMIT {limit}"
    res = market.query_questdb(sql, url=url)
    ...
    out = _envelope("ok", res["rows"], sql, res.get("caveats", []))
    out["tool_calls"] = [{"tool": "get_financial_metrics", "args": {"symbol": sym, "statement_type": statement_type}, "status": "ok", "row_count": len(res["rows"])}]
    return out
